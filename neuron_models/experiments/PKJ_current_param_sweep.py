...
if __name__ == "__main__":
    ...
    params = []
    for k in linspace(.1,20,100):
        for theta in linspace(.001,1,100):
            if k*theta < .1:
                params.append((k,theta))
    print len(params)
    results = pool.map(run_net, params)
    
    out_dir = out_dir = '/home/bill/research/data/neuron_models/molecular_layer/PKJ_current_param_sweep/%s/'%datetime.datetime.now().isoformat()
    os.makedirs(out_dir)
