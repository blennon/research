...
def run_net((w_pkj_pkj,w_pkj_bs,w_bs_bs,w_bs_pkj)):
    seed(os.getpid())
    print os.getpid()
    reinit()
    ...
    # Connections
    S_BS_PKJ[:,:] = 'minimum(abs(i-j),abs(abs(i-j)-N_PKJ))<=2' 
    S_BS_BS[:,:] = '(minimum(abs(i-j),abs(abs(i-j)-N_BS))<=2) & (i!=j)'
    ...
    MISI_BS = ISIHistogramMonitor(BS,bins=arange(0,160,2)*ms)
    
    MS_PKJ = SpikeMonitor(PKJ)
    MR_PKJ = PopulationRateMonitor(PKJ,bin=1*ms)
    MISI_PKJ = ISIHistogramMonitor(PKJ,bins=arange(0,160,2)*ms)
...
if __name__ == "__main__":
    ...
    results = pool.map(run_net, itertools.product(linspace(2.5,3.5,3),linspace(2.5,3.5,3),linspace(1.,2,3),linspace(1.,2.,3)))
    ...
    params = itertools.product(linspace(2.5,3.5,3),linspace(2.5,3.5,3),linspace(1.,2,3),linspace(1.,2.,3))
    with open(out_dir+'results.txt','w') as outf:
        outf.write('\t'.join(['w_pkj_pkj','w_pkj_bs','w_bs_pkj','w_bs_bs','bs_mean_firing_rate','bs_cv','pkj_mean_firing_rate','pkj_cv'])+'\n')
        for r in results:
            param = params.next()
